...
import { getActivityA, getSumActivityA } from '../Functions/RichMenu'
...
function Activity_a() {
    ...
    const [activityA, setActivityA] = useState([])
    const [searchQuery1, setSearchQuery1] = useState([])
    const [searchQuery2, setSearchQuery2] = useState([])
    const [searchQuery3, setSearchQuery3] = useState([])
    const [searchQuery4, setSearchQuery4] = useState([])
    const [searchQuery5, setSearchQuery5] = useState([])
    const [sumActivityA, setSumActivityA] = useState([])
    ...
    const sum_imp_A = sumActivityA.map((item) => parseInt(item.imp_A))
    const sum_impUU_A = sumActivityA.map((item) => parseInt(item.impUU_A))
    const sum_click_A = sumActivityA.map((item) => parseInt(item.click_A))
    const sum_clickUU_A = sumActivityA.map((item) => parseInt(item.clickUU_A))
    const sum_clickUURate_A = sumActivityA.map((item) => parseFloat(item.clickUURate_A))
    ...
    const handleFilter1 = (e) => {
        ...
        setSearchQuery1(activityA.filter(f => f.rich_date >= startDate && f.rich_date <= endDate))
    }

    const handleFilter2 = (e) => {
        const startDate = e[0].format('YYYY-MM-DD');
        const endDate = e[1].format('YYYY-MM-DD');
        setSearchQuery2(activityA.filter(f => f.rich_date >= startDate && f.rich_date <= endDate))
    }

    const handleFilter3 = (e) => {
        const startDate = e[0].format('YYYY-MM-DD');
        const endDate = e[1].format('YYYY-MM-DD');
        setSearchQuery3(activityA.filter(f => f.rich_date >= startDate && f.rich_date <= endDate))
    }

    const handleFilter4 = (e) => {
        const startDate = e[0].format('YYYY-MM-DD');
        const endDate = e[1].format('YYYY-MM-DD');
        setSearchQuery4(activityA.filter(f => f.rich_date >= startDate && f.rich_date <= endDate))
    }

    const handleFilter5 = (e) => {
        const startDate = e[0].format('YYYY-MM-DD');
        const endDate = e[1].format('YYYY-MM-DD');
        setSearchQuery5(activityA.filter(f => f.rich_date >= startDate && f.rich_date <= endDate))
    }
    ...
    const imp_A = searchQuery1.map((item) => item.imp_A)
    const impUU_A = searchQuery2.map((item) => parseInt(item.impUU_A))
    const click_A = searchQuery3.map((item) => parseInt(item.click_A))
    const clickUU_A = searchQuery4.map((item) => parseInt(item.clickUU_A))
    const clickUURate_A = searchQuery5.map((item) => parseFloat(item.clickUURate_A))
            ...
            <Layout>
                ...
                <Layout>
                    ...
                    >
                        ...
                        <h3 className='text-center'>สถิติข้อมูล RICH MENU นัดหมายออนไลน์</h3>
                            ...
                            <div className='row' style={{ margin: '5px' }}>
                                <div className='col'>
                                    ...
                                    >
                                        ...
                                        >
                                            {sum_imp_A.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                                        </h3>
                                    </div>
                                </div>
                                <div className='col'>
                                    ...
                                    >
                                        ...
                                        >
                                            {sum_impUU_A.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                                        </h3>
                                    </div>
                                </div>
                                <div className='col'>
                                    ...
                                    >
                                        ...
                                        >
                                            {sum_click_A.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                                        </h3>
                                    </div>
                                </div>
                                <div className='col'>
                                    ...
                                    >
                                        ...
                                        >
                                            {sum_clickUU_A.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                                        </h3>
                                    </div>
                                </div>
                                <div className='col'>
                                ...
                                <>
                                    <div className='row' style={{ margin: '10px' }}>
                                        <div className='card bg-white border rounded col m-1'>
                                            <div>
                                                <div>
                                                    <h4 className='text-center' style={{ margin: '10px' }}>จำนวนครั้งที่แสดง RICH MENU นัดหมายออนไลน์</h4>
                                                </div>
                                    ...
                                    <div className='row' style={{ margin: '10px' }}>
                                        <div className='card bg-white border rounded col m-1'>
                                            <div>
                                                <div>
                                                    <h4 className='text-center' style={{ margin: '10px' }}>จำนวนผู้ใช้ที่มีการเข้าถึง RICH MENU นัดหมายออนไลน์</h4>
                                                </div>
                                                <div style={{ marginRight: '40px', float: 'right' }}>
                                    ...
                                    <div className='row' style={{ margin: '10px' }}>
                                        <div className='card bg-white border rounded col m-1'>
                                            <div>
                                                <div>
                                                    <h4 className='text-center' style={{ margin: '10px' }}>จำนวนครั้งที่มีการคลิก RICH MENU นัดหมายออนไลน์</h4>
                                                </div>
                                                <div style={{ marginRight: '40px', float: 'right' }}>
                                    ...
                                    <div className='row' style={{ margin: '10px' }}>
                                        <div className='card bg-white border rounded col m-1'>
                                            <div>
                                                <div>
                                                    <h4 className='text-center' style={{ margin: '10px' }}>จำนวนผู้ใช้ที่คลิก RICH MENU นัดหมายออนไลน์</h4>
                                                </div>
                                                <div style={{ marginRight: '40px', float: 'right' }}>
                                    ...
                                    <div className='row' style={{ margin: '10px' }}>
                                        <div className='card bg-white border rounded col m-1'>
                                            <div>
                                                <div>
                                                    <h4 className='text-center' style={{ margin: '10px' }}>อัตราการคลิก RICH MENU นัดหมายออนไลน์</h4>
                                                </div>
                                                <div style={{ marginRight: '40px', float: 'right' }}>
