...
import { getActivityB, getSumActivityB } from '../Functions/RichMenu'
...
function Activity_b() {
    ...
    const [activityB, setActivityB] = useState([])
    const [searchQuery1, setSearchQuery1] = useState([])
    const [searchQuery2, setSearchQuery2] = useState([])
    const [searchQuery3, setSearchQuery3] = useState([])
    const [searchQuery4, setSearchQuery4] = useState([])
    const [searchQuery5, setSearchQuery5] = useState([])
    const [sumActivityB, setSumActivityB] = useState([])
    ...
    const sum_imp_B = sumActivityB.map((item) => parseInt(item.imp_B))
    const sum_impUU_B = sumActivityB.map((item) => parseInt(item.impUU_B))
    const sum_click_B = sumActivityB.map((item) => parseInt(item.click_B))
    const sum_clickUU_B = sumActivityB.map((item) => parseInt(item.clickUU_B))
    const sum_clickUURate_B = sumActivityB.map((item) => parseFloat(item.clickUURate_B))
    ...
    const handleFilter1 = (e) => {
        ...
        setSearchQuery1(activityB.filter(f => f.rich_date >= startDate && f.rich_date <= endDate))
    }

    const handleFilter2 = (e) => {
        const startDate = e[0].format('YYYY-MM-DD');
        const endDate = e[1].format('YYYY-MM-DD');
        setSearchQuery2(activityB.filter(f => f.rich_date >= startDate && f.rich_date <= endDate))
    }

    const handleFilter3 = (e) => {
        const startDate = e[0].format('YYYY-MM-DD');
        const endDate = e[1].format('YYYY-MM-DD');
        setSearchQuery3(activityB.filter(f => f.rich_date >= startDate && f.rich_date <= endDate))
    }

    const handleFilter4 = (e) => {
        const startDate = e[0].format('YYYY-MM-DD');
        const endDate = e[1].format('YYYY-MM-DD');
        setSearchQuery4(activityB.filter(f => f.rich_date >= startDate && f.rich_date <= endDate))
    }

    const handleFilter5 = (e) => {
        const startDate = e[0].format('YYYY-MM-DD');
        const endDate = e[1].format('YYYY-MM-DD');
        setSearchQuery5(activityB.filter(f => f.rich_date >= startDate && f.rich_date <= endDate))
    }
    ...
    const imp_B = searchQuery1.map((item) => item.imp_B)
    const impUU_B = searchQuery2.map((item) => parseInt(item.impUU_B))
    const click_B = searchQuery3.map((item) => parseInt(item.click_B))
    const clickUU_B = searchQuery4.map((item) => parseInt(item.clickUU_B))
    const clickUURate_B = searchQuery5.map((item) => parseFloat(item.clickUURate_B))
            ...
            <Layout>
                ...
                <Layout>
                    ...
                    >
                        ...
                        <h3 className='text-center'>สถิติข้อมูล RICH MENU ประวัติสุขภาพ</h3>
                            ...
                            <div className='row' style={{ margin: '5px' }}>
                                <div className='col'>
                                    ...
                                    >
                                        ...
                                        >
                                            {sum_imp_B.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                                        </h3>
                                    </div>
                                </div>
                                <div className='col'>
                                    ...
                                    >
                                        ...
                                        >
                                            {sum_impUU_B.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                                        </h3>
                                    </div>
                                </div>
                                <div className='col'>
                                    ...
                                    >
                                        ...
                                        >
                                            {sum_click_B.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                                        </h3>
                                    </div>
                                </div>
                                <div className='col'>
                                    ...
                                    >
                                        ...
                                        >
                                            {sum_clickUU_B.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                                        </h3>
                                    </div>
                                </div>
                                <div className='col'>
                                ...
                                <>
                                    <div className='row' style={{ margin: '10px' }}>
                                        <div className='card bg-white border rounded col m-1'>
                                            <div>
                                                <div>
                                                    <h4 className='text-center' style={{ margin: '10px' }}>จำนวนครั้งที่แสดง RICH MENU ประวัติสุขภาพ</h4>
                                                </div>
                                    ...
                                    <div className='row' style={{ margin: '10px' }}>
                                        <div className='card bg-white border rounded col m-1'>
                                            <div>
                                                <div>
                                                    <h4 className='text-center' style={{ margin: '10px' }}>จำนวนผู้ใช้ที่มีการเข้าถึง RICH MENU ประวัติสุขภาพ</h4>
                                                </div>
                                                <div style={{ marginRight: '40px', float: 'right' }}>
                                    ...
                                    <div className='row' style={{ margin: '10px' }}>
                                        <div className='card bg-white border rounded col m-1'>
                                            <div>
                                                <div>
                                                    <h4 className='text-center' style={{ margin: '10px' }}>จำนวนครั้งที่มีการคลิก RICH MENU ประวัติสุขภาพ</h4>
                                                </div>
                                                <div style={{ marginRight: '40px', float: 'right' }}>
                                    ...
                                    <div className='row' style={{ margin: '10px' }}>
                                        <div className='card bg-white border rounded col m-1'>
                                            <div>
                                                <div>
                                                    <h4 className='text-center' style={{ margin: '10px' }}>จำนวนผู้ใช้ที่คลิก RICH MENU ประวัติสุขภาพ</h4>
                                                </div>
                                                <div style={{ marginRight: '40px', float: 'right' }}>
                                    ...
                                    <div className='row' style={{ margin: '10px' }}>
                                        <div className='card bg-white border rounded col m-1'>
                                            <div>
                                                <div>
                                                    <h4 className='text-center' style={{ margin: '10px' }}>อัตราการคลิก RICH MENU ประวัติสุขภาพ</h4>
                                                </div>
                                                <div style={{ marginRight: '40px', float: 'right' }}>
